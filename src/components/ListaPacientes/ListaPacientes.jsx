...
import styles from './_listaPacientes.module.scss';
...
const ListaPacientes = () => {
    ...
    useEffect(() => {
        axios.get(`${process.env.REACT_APP_URL_BASE_BACKEND}/patients`, { headers: { "token": `${localStorage.getItem('token')}` } })
            .then(response => {
                if (response.data.payload) {
                    setListaPacientes(response.data.payload);
                        ...
                        listaPacientes?.length !== 0 ? (
                            ...
                            <table className={`${styles.list_ranking}`}>
                                ...
                                <tbody>
                                    {
                                        <tr>
                                            <td colSpan="2" className="center">No hay pacientes por el momento</td>
                                        </tr>
                                    }
                                </tbody>
                            </table>
